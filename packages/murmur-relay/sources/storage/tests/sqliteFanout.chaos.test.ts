...
function downgradeCurrentSchema(database: DatabaseSync, version: 3 | 4): void {
    database.exec("PRAGMA foreign_keys = OFF");
    database.exec("BEGIN IMMEDIATE");
    try {
        database.exec(`
            CREATE TABLE murmur_queue_global_legacy (
            ...
            INSERT INTO murmur_queue_global_legacy
                (singleton, last_event_id, pending_items, pending_bytes, pending_references)
            SELECT singleton, last_event_id, pending_items, pending_bytes, pending_references
            FROM murmur_queue_global;

            CREATE TABLE murmur_queues_legacy (
            ...
            INSERT INTO murmur_queues_legacy
            ...
            CREATE TABLE murmur_queue_references_legacy (
                recipient BLOB NOT NULL REFERENCES murmur_queues_legacy(recipient)
                    ON DELETE CASCADE,
            ...
            INSERT INTO murmur_queue_references_legacy
                (recipient, event_id, sender, delivery_id, encoded_bytes, admission_principal)
            SELECT recipient, event_id, sender, delivery_id, encoded_bytes, admission_principal
            FROM murmur_queue_references;

            DROP TABLE murmur_queue_references;
            DROP TABLE murmur_queues;
            DROP TABLE murmur_queue_global;
            ALTER TABLE murmur_queue_global_legacy RENAME TO murmur_queue_global;
            ALTER TABLE murmur_queues_legacy RENAME TO murmur_queues;
            ALTER TABLE murmur_queue_references_legacy RENAME TO murmur_queue_references;
        ...
        if (version === 3) {
            database.exec(`
                DROP TABLE murmur_invitation_revocations;
                CREATE TABLE murmur_invitations_legacy (
                ...
                INSERT INTO murmur_invitations_legacy
                ...
                DROP TABLE murmur_invitations;
                ALTER TABLE murmur_invitations_legacy RENAME TO murmur_invitations;
        ...
        database
            .prepare("UPDATE murmur_queue_schema SET version = ? WHERE singleton = 1")
            .run(BigInt(version));
...
describe("SQLite relay and durable fanout chaos", () => {
    ...
    test.each([3, 4] as const)(
        "SQL-08 v%i migrates pending rows to v5 with deterministic sequences",
        async (version) => {
            await withDatabasePath(async (path) => {
                const sender = secret(16 + version);
                const bob = identity(secret(20 + version));
                const carol = identity(secret(24 + version));
                const bundle = new TextEncoder().encode(`v${version} invitation`);
                const digest = sha256(bundle);
                let store = new SqliteRelayStore(path);
                for (let index = 0; index < 3; index += 1) {
                    await store.publish(
                        chaosDelivery(sender, recipients(bob, carol), { id: 30 + index }),
                        NOW,
                        LIMITS,
                        PRINCIPAL,
                    );
                ...
                const ids = (await queue(store, bob)).deliveries.map(
                    (delivery) => delivery.eventId,
                );
                await store.close();

                const database = new DatabaseSync(path);
                downgradeCurrentSchema(database, version);
                database.close();

                store = new SqliteRelayStore(path);
                const migrated = await queue(store, bob);
                expect(migrated.deliveries.map((delivery) => delivery.eventId)).toEqual(ids);
                expect(migrated.deliveries.map((delivery) => delivery.sequence)).toEqual([1, 2, 3]);
                expect(migrated).toMatchObject({
                    headSequence: 3,
                    acknowledgedSequence: 0,
                });
                expect(migrated.generation).toHaveLength(32);
                expect(await store.readInvitation(digest, NOW)).toEqual({
                    bundle,
                    expiresAt: NOW + 60_000,
                });
                expect((await queue(store, carol)).deliveries).toHaveLength(3);
                await store.close();

                store = new SqliteRelayStore(path);
                expect(normalizePage(await queue(store, bob))).toEqual(normalizePage(migrated));
                await store.close();
            });
        },
    );
