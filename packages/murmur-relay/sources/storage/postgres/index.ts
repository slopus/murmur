...
export class PostgresRelayStore implements RelayStore {
        ...
        await transaction.query(
            `INSERT INTO murmur_queue_deliveries
                    (sender, delivery_id, event_id, fingerprint, delivery_json,
                     encoded_bytes, expires_at, sender_account, owner_account, session_id)
    ...
    async readDeviceAccount(deviceKey: Uint8Array): Promise<Uint8Array | undefined> {
        ...
        const result = await this.#database.query<{ account_key: unknown }>(
    ...
    async deleteAccountState(
        accountKey: Uint8Array,
        requestId: string,
        now: number,
    ): Promise<void> {
        this.#assertOpen();
        if (accountKey.length !== 32 || requestId.length < 1) {
            throw new Error("Invalid account deletion");
        }
        ...
        await this.#database.transaction(async (transaction) => {
            ...
            await transaction.query(
                `DELETE FROM murmur_account_deletion_nonces WHERE created_at < $1`,
                [(now - ACCOUNT_DELETION_NONCE_RETENTION_MILLISECONDS).toString()],
            );
            const replay = await transaction.query<{ request_id: unknown }>(
                `SELECT request_id FROM murmur_account_deletion_nonces
                 WHERE account_digest = $1 AND request_id = $2`,
                [accountDigest, requestId],
            );
            if (replay.rows.length > 0) {
            ...
            await transaction.query(
                `INSERT INTO murmur_account_deletion_nonces
                    (account_digest, request_id, created_at) VALUES ($1, $2, $3)`,
            ...
            const inboxes = [
                accountKey,
                ...devices.rows.map((row) => copyBytes(row.device_key, "account device key")),
            ];
            const inboxParameters = inboxes.map((_, index) => `$${index + 1}::bytea`).join(", ");
            const inboxUsage = await transaction.query<{ reference_count: unknown }>(
                `SELECT COUNT(*) AS reference_count FROM murmur_queue_references
                 WHERE recipient IN (${inboxParameters})`,
                inboxes,
            );
            await transaction.query(
                `DELETE FROM murmur_queues WHERE recipient IN (${inboxParameters})`,
                inboxes,
            );
            const inboxUsageRow = inboxUsage.rows[0];
            if (inboxUsageRow === undefined) throw new Error("Missing account inbox usage result");
            await transaction.query(
                `UPDATE murmur_queue_global
                 SET pending_references = pending_references - $1 WHERE singleton = 1`,
                [bigintColumn(inboxUsageRow.reference_count).toString()],
            );
            const orphanUsage = await transaction.query<{
                item_count: unknown;
                byte_count: unknown;
            }>(
                `SELECT COUNT(*) AS item_count, COALESCE(SUM(encoded_bytes), 0) AS byte_count
                 FROM murmur_queue_deliveries AS delivery
                 WHERE NOT EXISTS (
                     SELECT 1 FROM murmur_queue_references AS reference
                     WHERE reference.sender = delivery.sender
                       AND reference.delivery_id = delivery.delivery_id
            ...
            const orphanUsageRow = orphanUsage.rows[0];
            if (orphanUsageRow === undefined) throw new Error("Missing orphan usage result");
            await transaction.query(
                `DELETE FROM murmur_queue_deliveries AS delivery
                 WHERE NOT EXISTS (
                     SELECT 1 FROM murmur_queue_references AS reference
            ...
            await transaction.query(
                `UPDATE murmur_queue_global
                 SET pending_items = pending_items - $1, pending_bytes = pending_bytes - $2
                 WHERE singleton = 1`,
                [
                    bigintColumn(orphanUsageRow.item_count).toString(),
                    bigintColumn(orphanUsageRow.byte_count).toString(),
                ],
            );
            await transaction.query(
                `DELETE FROM murmur_session_deletion_nonces WHERE owner_account = $1`,
                [accountKey],
            );
            await transaction.query(`DELETE FROM murmur_device_rosters WHERE account_key = $1`, [
                accountKey,
            ]);
        });
    }
    ...
    async #deleteOwnedAccountDeliveries(
        transaction: PostgresQuery,
        accountKey: Uint8Array,
    ): Promise<number> {
        const usage = await transaction.query<{
        ...
        const usageRow = usage.rows[0];
        if (usageRow === undefined) throw new Error("Missing account deletion usage result");
        const affected = await transaction.query<{
            recipient: unknown;
            item_count: unknown;
            byte_count: unknown;
