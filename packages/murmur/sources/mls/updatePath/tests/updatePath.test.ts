import { equalBytes, generateIdentityKeyPair, hashBytes, utf8Encode } from "../../internal.js";
import { describe, expect, it } from "vitest";
import { type IdentityKeyPair } from "../../internal.js";
import {
    createMlsKeyPackage,
...
describe("MLS UpdatePath", () => {
    ...
    it("distributes the same commit secret to another member", () => {
        ...
        const forgedUnsigned: Omit<MlsLeafNode, "signature"> = {
            ...
            signatureKey: attacker.publicKey,
            credential: { identity: attacker.publicKey },
    ...
    it("excludes a removed member while a retained member advances", () => {
        ...
        const created = createMlsUpdatePath({
            tree: candidate,
            sender: 0,
            signingSecretKey: identities[0]!.secretKey,
            provisionalContext: provisional(groupId),
            authenticateCredential,
        });
