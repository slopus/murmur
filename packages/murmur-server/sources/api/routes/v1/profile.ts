...
export async function profileRoutes(app: Fastify) {
    ...
    app.post(
        ...
        async (request, reply) => {
            const userId = getAuthUserId(request);
            const { username, description, avatar, timestamp, signature } = request.body;
            ...
            if (existingByUser && existingByUser.username !== normalizedUsername) {
                ...
                profile = await db.publicProfile.update({
                    ...
                    data: {
                        description: normalizedDescription,
                        ...(avatar ? { avatarImage, avatarThumbhash } : {}),
                    },
                });
            } else {
                profile = await db.publicProfile.create({
