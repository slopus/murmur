...
export class MurmurDatabase {
    ...
    private runMigrations(): void {
        const appliedRows = this.db.prepare("SELECT id FROM migrations").all() as Array<{
            id: string;
        }>;
    ...
    private ensureContactSchema(): void {
        const columns = this.db.prepare("PRAGMA table_info(contacts)").all() as Array<{
            name: string;
        }>;
    ...
    deleteContact(identityKey: string): void {
        ...
        const deleteContact = this.db.prepare("DELETE FROM contacts WHERE identity_key = ?");
