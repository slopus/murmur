...
import { stringToBytes, bytesToString, constantTimeEqual } from "./encryption/crypto/utils.js";
...
describe("Full Protocol Integration", () => {
    it("should complete X3DH → Double Ratchet conversation", () => {
        ...
        expect(constantTimeEqual(aliceX3DHResult.sharedSecret, bobX3DHResult.sharedSecret)).toBe(
            true,
        );
        ...
        const aliceSession: Session = {
            ratchetState: initializeAlice(
                aliceX3DHResult.sharedSecret,
                aliceX3DHResult.bobSignedPreKey,
            ),
    ...
    it("should persist and restore session state", () => {
        ...
        const bobKeyStore = initializeKeyStore(10);
        const aliceKeyStore = initializeKeyStore(10);
        const bobBundle = createPreKeyBundle(bobKeyStore.identityKeyPair, bobKeyStore.signedPreKey);
    ...
    it("should establish multiple independent sessions", () => {
        ...
        const aliceRatchet = initializeAlice(aliceX3DH.sharedSecret, aliceX3DH.bobSignedPreKey);
        const charlieRatchet = initializeAlice(
            charlieX3DH.sharedSecret,
            charlieX3DH.bobSignedPreKey,
        );

        const bobAliceRatchet = initializeBob(
            bobAliceX3DH.sharedSecret,
            bobKeyStore.signedPreKey.keyPair,
        );
        const bobCharlieRatchet = initializeBob(
            bobCharlieX3DH.sharedSecret,
            bobKeyStore.signedPreKey.keyPair,
        );
        ...
        expect(bytesToString(ratchetDecrypt(bobCharlieRatchet, charlieMsg))).toBe(
            "Hi Bob, from Charlie",
        );
