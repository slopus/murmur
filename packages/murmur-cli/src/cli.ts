...
function printAccountSummary(
    prefix: string,
    account: { profileSecretKey: string; firstName: string; lastName?: string },
): void {
        ...
        switch (parsed.command) {
            case "sign-in": {
                ...
                if (initialized) {
                    ...
                    if (authError) {
                        logger.warn(
                            `Note: server login failed (${authError}). Run \`murmur delete-account --confirm\` then \`murmur sign-in\` to re-register.`,
                        );
                    }
                    return;
                }

                const firstName = requireStringOption(
                    parsed.options,
                    "first-name",
                    "MURMUR_FIRST_NAME",
                );
            ...
            case "me": {
                ...
                const profileSecretKeyBytes = decodeStoredProfileSecretKey(
            ...
            case "delete-account": {
                await requireInitialized(getEngine());
                const confirmed = readBooleanOption(
                    parsed.options,
                    "confirm",
                ...
                if (!confirmed) {
                    throw new Error(
                        "Account deletion requires --confirm or MURMUR_CONFIRM_DELETE=true",
                    );
                }
            ...
            case "send": {
                ...
                const stored = await getEngine().sendMessage(
                    contact.identityKey,
                    message,
                    messageId,
                    attachments,
                ...
                logger.info(
                    `Sent ${stored.id} to ${formatContact(contact)} at ${formatTimestamp(stored.createdAt)}`,
                );
                return;
            }
            case "contacts": {
                ...
                if (action === "add") {
                    ...
                    const contact =
                        await getEngine().addContactByProfileSecret(profileSecretBase64);
                ...
                if (action === "block" || action === "unblock") {
                    ...
                    if (!profileSecret) {
                        throw new Error(
                            `Missing contact ID. Usage: murmur contacts ${action} <id>`,
                        );
                    }
                    const profileSecretBytes = decodeProfileSecretKey(profileSecret);
                    const profileSecretBase64 = encodeBase64(profileSecretBytes, "base64url");
            ...
            case "configure": {
                ...
                if (setting) {
                    ...
                    if (!key || !value) {
                        throw new Error(
                            "Invalid setting. Usage: murmur configure permissions:default-allow|permissions:default-deny|message-max-chars:<number>|attachment-max-bytes:<number>",
                        );
                    }
                    if (key === "permissions") {
            ...
            case "profile": {
                ...
                const profileSecret =
                    positionalSecret ??
                    requireStringOption(parsed.options, "profile-secret", "MURMUR_PROFILE_SECRET");
                const profileSecretBytes = decodeProfileSecretKey(profileSecret);
            ...
            case "public-profile": {
                ...
                if (!action) {
                    throw new Error(
                        "Missing action. Usage: murmur public-profile get <username> | murmur public-profile commit --username <name> --description <text> [--avatar <path> --thumbhash <hash>]",
                    );
                }
                if (action === "get") {
                    ...
                    if (!username) {
                        throw new Error(
                            "Missing username. Usage: murmur public-profile get <username>",
                        );
                    }
                    const api = new MurmurApi(apiBaseUrl);
                ...
                if (action === "commit") {
                    await requireInitialized(getEngine());
                    const username = requireStringOption(
                        parsed.options,
                        "username",
                        "MURMUR_PUBLIC_USERNAME",
                    );
                    const description = requireStringOption(
                        parsed.options,
                        "description",
                        "MURMUR_PUBLIC_DESCRIPTION",
                    );
                    const avatarPath = readStringOption(
                        parsed.options,
                        "avatar",
                        "MURMUR_PUBLIC_AVATAR",
                    );
                    const thumbhash = readStringOption(
                        parsed.options,
                        "thumbhash",
                        "MURMUR_PUBLIC_THUMBHASH",
                    );
                    ...
                    const publicProfile = await getEngine().commitPublicProfile(
                        username,
                        description,
                        avatar,
                    );
            ...
            case "sync": {
                ...
                try {
                    ...
                    while (!controller.signal.aborted) {
                        ...
                        try {
                            await getEngine().streamMessages(
                                async (event) => {
                                    ...
                                    if (getEngine().hasMessage(messageId)) {
                                        logger.debug(
                                            `Realtime message already stored: ${messageId}`,
                                        );
                                        return;
                                    }
                        ...
                        if (!controller.signal.aborted) {
                            logger.warn(
                                `Retrying realtime stream in ${Math.round(backoffMs / 1000)}s...`,
                            );
                            try {
            ...
            case "hooks": {
                ...
                if (action === "add") {
                    ...
                    if (!typeRaw || !hookPath) {
                        throw new Error(
                            "Missing hook info. Usage: murmur hooks add message <path> [--arg <value> ...]",
                        );
                    }
