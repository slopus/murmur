...
describe("MockServer", () => {
    ...
    describe("Messages", () => {
        ...
        it("should fail send to non-existent recipient", () => {
            expect(() =>
                server.sendMessage(aliceToken, "msg-1", "nonexistent", "blob", "sig"),
        ...
        it("should fail send with duplicate message ID", () => {
            ...
            expect(() =>
                server.sendMessage(aliceToken, "msg-1", "bob-id", "blob2", "sig2"),
    ...
    describe("Profiles", () => {
        ...
        it("should update profile", () => {
            server.updateProfile(aliceToken, "new-profile-key", "new-sig", "new-encrypted-profile");
    ...
    describe("Prekey Bundles", () => {
        ...
        it("should fail to get bundle for user without bundle", () => {
            expect(() => server.getPreKeyBundle(aliceToken, "bob-id")).toThrow(
                "No prekey bundle available",
            );
        });

        it("should fail to get bundle for non-existent user", () => {
            expect(() => server.getPreKeyBundle(aliceToken, "nonexistent")).toThrow(
                "User not found",
            );
        });
    });
...
describe("createMockApi", () => {
    ...
    it("should register via mock API", async () => {
        const result = await api.register("alice-id", new Uint8Array(32), "ppk", "sig", "ep");
