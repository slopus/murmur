...
`delivery`; the relay's own behavior belongs in `relay`; public identity, the account
secret, the device roster, and the directory belong in `identity`; optional
typed synchronization capabilities belong in `services`.
